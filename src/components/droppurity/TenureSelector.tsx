    ...
    <div className="w-full overflow-visible">
      <div className={`mt-4 pt-2 grid gap-3 sm:gap-4 w-full overflow-visible ${tenureOptions.length === 2 ? 'grid-cols-2' : tenureOptions.length === 1 ? 'grid-cols-1' : 'grid-cols-3'}`}>
        {tenureOptions.map((option) => {
          ...
          return (
            <div key={option.id} className="flex flex-col items-center justify-end">
              ...
              <button
                ...
                className={`
                  ...
                  rounded-xl border-2 transition-all duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-offset-2
                  ${isSelected 
                    ? 'bg-gradient-to-br from-dynamic-accent to-blue-600 text-white border-transparent shadow-lg shadow-dynamic-accent/30 scale-[1.02]' 
                    : 'bg-card border-border hover:border-dynamic-accent/50 hover:bg-muted/30 focus:ring-dynamic-accent hover:shadow-sm scale-100'
                  }
                `}
              >
